...
{
    if (glm::any(glm::lessThan(glm::round(coord - 0.5f), glm::vec3(0)))
        || glm::any(glm::lessThan(glm::round(coord + 0.5f), glm::vec3(0)))
        || glm::any(glm::greaterThanEqual(glm::round(coord - 0.5f), glm::vec3(m_dim)))
        || glm::any(glm::greaterThanEqual(glm::round(coord + 0.5f), glm::vec3(m_dim))))
        return 0.0f;

    int nearZ = glm::round(coord.z - 0.5f);
    int farZ = glm::round(coord.z + 0.5f);
    float near = biLinearInterpolate(glm::vec2(coord.x, coord.y), nearZ);
    float far = biLinearInterpolate(glm::vec2(coord.x, coord.y), farZ);

    float zFactor = coord.z - floorf(coord.z);

    return linearInterpolate(near, far, zFactor);
}
...
{
    float topLeft = getVoxel(glm::round(xyCoord.x - 0.5f), glm::round(xyCoord.y - 0.5f), z);
    float topRight = getVoxel(glm::round(xyCoord.x + 0.5f), glm::round(xyCoord.y - 0.5f), z);
    float bottomLeft = getVoxel(glm::round(xyCoord.x - 0.5f), glm::round(xyCoord.y + 0.5f), z);
    float bottomRight = getVoxel(glm::round(xyCoord.x + 0.5f), glm::round(xyCoord.y + 0.5f), z);

    float xFactor = xyCoord.x - floorf(xyCoord.x);
    float yFactor = xyCoord.y - floorf(xyCoord.y);

    float topInterpolate = linearInterpolate(topLeft, topRight, xFactor);
    float bottomInterpolate = linearInterpolate(bottomLeft, bottomRight, xFactor);
    float bilinearInterpolate = linearInterpolate(topInterpolate, bottomInterpolate, yFactor);
